...
impl WorldGenerator {
    ...
    /// ```
    pub fn generate_and_save(&mut self, file_path: &str) -> std::io::Result<()>{
        let w = self.gen();
        self.save(file_path, w)
    }
    ...
    /// the specified file path with a `.zst` extension.
    ///
    ...
    pub fn save(&mut self, file_path: &str, world: GenResult) -> std::io::Result<()> {
        let r = SerializedWorld {
            settings: self.clone(),
            world,
        }.serialize(11)?;

        let mut file = File::create(format!("{file_path}.zst"))?;
        file.write_all(&r)
    }


    pub fn export(&mut self, world: GenResult) -> std::io::Result<Vec<u8>> {
        SerializedWorld {
            settings: self.clone(),
            world,
        }.serialize(22)
...
pub type GenResult = (TileMatrix, (usize, usize), EnvironmentalConditions, f32, Option<HashMap<Content, f32>>);
